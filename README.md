...
All required packages are listed in the requirements.txt file. The dataset is imported in utils.py, followed by transform_data function, which voxelizes the input using an appropriate grid-point size. Based on our experience, setting the voxel size to approximately one-fourth of the typical bond length provides an optimal balance between spatial resolution and computational efficiency.
